# THAIDUY.DIGITAL — FULL HANDOFF — 2026-09-23
...
> Runtime host: `VPS-ARM`
> Timezone: `Asia/Ho_Chi_Minh (+07)`
> This file supersedes runtime/Git-state headers in every older handoff.
> Historical implementation sections from the 22/09 full handoff are carried forward below.
...
At closeout on **2026-09-23**:

- branch: `main`
- HEAD before this handoff-doc commit: `076776d56a55d287d043cefdd4cc3625645f1bd0`
- HEAD subject: `fix: polish command overlay and music signal surfaces`
- `origin/main`: same commit
- code worktree before creating this handoff: **CLEAN**
- runtime: **Next.js dev / bun dev on port 3000**
- dev runtime had dropped immediately before handoff and was restarted successfully
- selected smoke routes are healthy after restart

Selected local smoke after restart:
...
Do **not** trust stale dirty-worktree warnings or old HEAD values from the 18/09, 20/09, or 22/09 handoffs.

Historical handoffs retained:

- `HANDOFF_2026-09-18_FULL.md`
- `HANDOFF_2026-09-20_FULL.md`
- `HANDOFF_2026-09-22_FULL.md`

Use this `HANDOFF_2026-09-23_FULL.md` first.
...
- `29724517` — style: balance homepage manifesto and live ticker
- `33e22a02` — style: sharpen homepage manifesto typography
- `7eb8de00` — style: add control chart hover tooltip
- `3ecb9c8e` — feat: add weekly control activity chart
- `adbec05f` — style: add control shortcuts and fix home overflow
- `db5d0036` — style: spin about math refresh
- `5b1a6bb8` — style: lift about arrows on hover
- `bfca834c` — fix: improve humming instrument timbres
- `1e4fa0d8` — style: improve writing dark readability
- `2ec9df7e` — style: soften home field note hover
- `8b20f15b` — style: align writing highlight rail
- `d5557c7f` — style: polish writing highlight rail
- `4f3bad53` — revert: simplify writing highlight treatment
- `2177018f` — style: emphasize writing highlights on desktop
- `cf47517e` — style: slim stack room accent beams

Golden lineage is still valuable historically, but `main` is now substantially ahead of the 20/09 golden tag.

## 0.3 Owner direction / UX style

Current focus is the public website + `/control` surface.

Owner preferences that repeatedly matter:

- restrained, purposeful motion instead of decorative animation
- subtle hover; avoid card jumping and aggressive drop shadows
- preserve the clean systems-lab visual language
- do not expose internal implementation jargon publicly when it adds no value
- real state/data is preferred over simulations or decorative fake counters
- commit completed work instead of leaving large dirty diffs
- do not broad reset/clean/revert unless explicitly requested
- when a visual treatment is rejected, roll back only that treatment and preserve accepted surrounding work

Android / Scrobble Hub remains paused unless explicitly requested.

## 0.4 Secrets

Never print, paste into chat, commit, or expose:

- `.env.local`
- Better Auth / Google OAuth secrets
- Redis / R2 credentials
- GitHub tokens
- Discord webhook URLs
- Unsplash credential material
- root secret-note files

About contact uses Discord server-side; never expose the webhook to the browser.
...
Current code HEAD before this handoff-doc commit:

`076776d56a55d287d043cefdd4cc3625645f1bd0`

Current `origin/main` before this handoff-doc commit:

`076776d56a55d287d043cefdd4cc3625645f1bd0`

Worktree before creating this handoff: **CLEAN**

Historical golden tag retained:

`golden-2026-09-20-public-site -> adae83de`

Normal closeout pattern:

1. run targeted QA
2. inspect diff / `git diff --check`
3. secret-scan staged diff
4. commit
5. push `origin main`
...
Serving mode remains development runtime:

- package script: `bun dev`
- wrapper: `scripts/dev-with-logrotate.sh`
- Next.js dev on port `3000`
- logrotate watcher is intentional
- NetBird must be left alone unless explicitly requested

Closeout event:

- port 3000 was found down during final handoff inspection
- runtime was restarted through the repo's normal `bun dev` path
- Next reported ready in ~0.5s
- `ss` confirmed port 3000 listening
- selected local smoke routes returned 200

If runtime is down next session:
...
Then verify `ss -ltnp | grep ':3000 '`, the dev log, and a local curl.
Final standalone/container production deployment has **not** replaced this dev runtime.

---

# 3. CURRENT QA / ACCEPTANCE STATE

Latest code commit `076776d5` is the final code baseline before this handoff.

Known validation from the final implementation sequence:

- TypeScript PASS
- layout invariant audit PASS
- theme contrast audit PASS
- `git diff --check` PASS
- local/public home smoke PASS after code changes
- final runtime restart smoke on 23/09 PASS for all selected routes above
...
The old `44/44 routes PASS` value belongs to an earlier full production build. Do not assume that route count is current after Discuss and later work; run a fresh production build when a release checkpoint is needed.

## 3.1 Immediate owner visual acceptance still pending

The owner requested these four fixes, then asked for this handoff before visually re-testing them. They are implemented in `076776d5`; verify them first next session:

1. **Command palette**
   - opens centered
   - outside click closes anywhere on page
   - ESC closes
   - overlay is no longer trapped by sticky-header containing block

2. **Homepage public-data ticker**
   - continuous motion
   - no visible pause/seam at loop reset
   - edge fade remains subtle

3. **Homepage LIVE INSTRUMENT**
   - right semantic column is wide enough
   - palette/motion/energy/valence text is not clipped
   - wave area still feels balanced

4. **Header music-wave tooltip**
   - title is one line
   - short title remains static
   - long title pans immediately on open
   - right-to-left pan reveals the full ending
   - both ends fade while panning
   - close/reopen restarts from the beginning
   - metadata does not wrap awkwardly

Reference read for the marquee behavior:
`/home/ubuntu/thaiduy.old/frontend/src/components/layout/now-playing-waveform.tsx`

Current site ports its `scrollWidth - clientWidth` measurement but intentionally removes the old initial hold.
...
# 99. NEXT SESSION START SEQUENCE — 2026-09-23

Start here before changing code:

1. `cd /home/ubuntu/n8n2erpnext/thaiduy.digital`
2. read this file, especially sections `0`–`3`
3. run `git status --short`
4. run `git log -8 --oneline`
5. confirm `HEAD == origin/main`
6. confirm port 3000; restart `bun dev` only if needed
7. smoke `/`, `/writing`, `/stack`, `/music-sensor`, `/discuss`, `/control/login`
8. visually verify the four pending items in section `3.1`
9. preserve accepted NAV / Music Sensor / Stack / Writing / Discuss behavior unless owner requests a change
10. after accepted work: QA → secret scan → commit → push → clean worktree

If the owner says simply **“tiếp”**, continue from section `3.1` visual acceptance first, then proceed with the next requested website polish.
