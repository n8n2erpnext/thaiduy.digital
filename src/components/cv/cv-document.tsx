...
function DocumentMeta({ snapshot, verifyUrl, qrSvg }:{ snapshot:CvSnapshot; verifyUrl:string; qrSvg:string }) {
  const vi=snapshot.locale==='vi'
  return (
    <footer className="cv-document-meta">
      <span className="cv-document-identity">
        <b>{vi?'Mã tài liệu':'Document ID'}</b> {snapshot.id}
        <i>·</i>
        <b>FP</b> {shortCvFingerprint(snapshot.contentHash)}
      </span>
      <span><b>{vi?'Phát hành':'Issued'}</b> {formatCvIssuedAt(snapshot.issuedAt,snapshot.locale)} GMT+7</span>
      <a className="cv-document-verify" href={verifyUrl}>
        <span>
          <b>{vi?'Xác thực':'Verify'}</b>
          <em>thaiduy.digital/verify</em>
        </span>
        <span className="cv-document-qr" aria-hidden="true" dangerouslySetInnerHTML={{ __html:qrSvg }} />
      </a>
    </footer>
  )
}

export async function CvDocument({ snapshot, verified=false }:Props) {
  const { content }=snapshot
  const vi=snapshot.locale==='vi'
  const verifyUrl=`https://thaiduy.digital/verify?id=${encodeURIComponent(snapshot.id)}`
  const qrSvg=await QRCode.toString(verifyUrl,{
    type:'svg',
    errorCorrectionLevel:'Q',
    margin:0,
    width:128,
    color:{ dark:'#243128', light:'#f7f8f4' },
  })
    ...
    <article className="cv-document" data-cv-document>
      <section className="cv-sheet cv-sheet-one">
        <header className="cv-doc-header">
          <div className="cv-running-head">
            <div className="cv-running-copy">
            ...
            <div className="cv-page-number" aria-label={vi?'Trang 1 trên 2':'Page 1 of 2'}>
              <b>01</b><span>/ 02</span>
            </div>
          </div>
        ...
        <DocumentMeta snapshot={snapshot} verifyUrl={verifyUrl} qrSvg={qrSvg} />
      </section>

      <section className="cv-sheet cv-sheet-two">
        <header className="cv-running-head cv-running-head-page-two">
          <div className="cv-running-copy">
            <strong>CURRICULUM VITAE</strong>
            <span>{content.name} · {vi?'KINH NGHIỆM & HỆ THỐNG':'EXPERIENCE & SYSTEMS'}</span>
          </div>
          <div className="cv-page-number" aria-label={vi?'Trang 2 trên 2':'Page 2 of 2'}>
            <b>02</b><span>/ 02</span>
          </div>
        </header>
