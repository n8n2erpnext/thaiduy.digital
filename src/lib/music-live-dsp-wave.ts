...
import { applyAcousticStage,resolveAcousticStage,type AcousticStageName } from '@/lib/music-acoustic-stage'
...
export type LiveDspWaveStats={
  ...
  stereoWidth:number;sharpness:number;stageLift:number;stageFocus:number;stage:AcousticStageName
}
...
export function liveDspWavePath({layer,frames,now,delayMs=900,xStart=0,width,centerY,amplitude,points=62}:{layer:MusicLayerName;frames:BufferedMusicDspFrame[];now:number;delayMs?:number;xStart?:number;width:number;centerY:number;amplitude:number;points?:number}):{path:string;stats:LiveDspWaveStats} {
  const flat=():{path:string;stats:LiveDspWaveStats}=>({
    ...
    stats:{activity:0,crest:0,latest:0,baseline:0,gain:0,motion:0,stereoWidth:0,sharpness:0,stageLift:1,stageFocus:0,stage:'balanced'},
  })
  ...
  return {
    ...
    stats:{
      ...
      stereoWidth:controls.stereoSpread,sharpness:controls.sharpness,
      stageLift:staged.lift,stageFocus:stage.focus,stage:stage.dominant,
    },
  }
}
