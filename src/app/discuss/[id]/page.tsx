...
export default async function DiscussThreadPage({params,searchParams}:Props) {
  ...
  return (
    <div className="site-shell">
      ...
      <main className="discuss-thread-page">
        ...
        <article className="discuss-thread-detail">
          <header>
            <span>
              {locale==='vi'?'DISCUSS / CHỦ ĐỀ':'DISCUSS / THREAD'}
              {data.thread.pinned?' · PINNED':''}
              {data.thread.locked?' · LOCKED':''}
            </span>
          ...
          {data.thread.bodyHtml ? (
            <div
              className="discuss-thread-message discuss-post-body"
        ...
        <section className="discuss-replies">
            ...
            <article className="discuss-reply" key={reply.id}>
              ...
              <div>
                ...
                {reply.parentReplyId && reply.parentAuthorName && (
                  <blockquote className="discuss-reply-reference">
          ...
          {data.replyPagination.pages>1 && (
            <nav className="discuss-pagination" aria-label={locale==='vi'?'Phân trang phản hồi':'Reply pagination'}>
              <Link
                href={data.replyPagination.page<=2?'/discuss/'+id+'#replies':'/discuss/'+id+'?page='+(data.replyPagination.page-1)+'#replies'}
              ...
              <Link
                href={'/discuss/'+id+'?page='+(data.replyPagination.page+1)+'#replies'}
        ...
        {data.thread.locked ? (
          <div className="discuss-locked-notice">
