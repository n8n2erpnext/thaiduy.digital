...
# Writing highlight desktop polish

Public /writing highlight treatment was adjusted after visual QA.

Problems found:

- the HIGHLIGHT / 2 / 2 section header looked visually detached
- highlighted posts were functionally promoted but desktop cards looked almost identical to normal post cards
- mobile already felt sufficiently prominent because highlights collapse to one column

Changes:

- highlight section header is now a structured rail:
  - signal dot + HIGHLIGHT label
  - theme-aware separator line
  - compact count chip
- desktop/tablet highlight grid gap increased slightly
- highlighted desktop cards now:
  - have a slightly stronger theme-aware surface/border
  - use a subtle top accent rail
  - gain a little more height
  - use slightly larger title typography
  - use slightly roomier body padding
  - lift 2px on hover
- Normal/day mode highlight surface was strengthened separately so featured cards do not visually collapse into regular cards
- mobile single-column behavior remains essentially unchanged
