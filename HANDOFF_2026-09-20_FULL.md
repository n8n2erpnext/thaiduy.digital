...
# 55. 2026-09-20 SESSION HANDOFF — READ THIS FIRST

This file is a continuation of the full 2026-09-18 handoff copied above.
Sections 1–54 remain the historical source of truth for CMS, Writing, Music Sensor, Stack runtime, tags, search, theme, typography, and the published article series.

The current session focused heavily on public UI direction, especially /stack.
The owner explicitly wants the next session to CONTINUE FROM THE CURRENT STATE, not re-run earlier redesign experiments.
...
- the worktree is intentionally very dirty with substantial uncommitted product work

CRITICAL:
- DO NOT git reset --hard.
- DO NOT git clean.
- DO NOT checkout/restore broad paths from HEAD.
- DO NOT assume origin/main contains the current product.
- Read this handoff and inspect the dirty worktree before changing anything.

# 56. CURRENT RUNTIME / PROCESS STATE

At handoff time:
- no `bun dev` / `next dev` process is currently running
- public production route https://thaiduy.digital/stack returns HTTP 200
- last full production build passed
- Next.js: 16.3.5
- React: 19.2.8
- Bun package manager: 1.3.12
...
Useful QA scripts in package.json:
- bun run theme:check
- bun run layout:check
- bun run typography:check
- bun run build
- bun run stack:refresh
- bun run stack:refresh:requested

Latest QA counters:
- Theme audit Normal-mode selectors: 476
- Theme uncovered bright text: 0
- Theme uncovered dark surfaces: 0
- Theme uncovered light borders: 0
- Theme uncovered light SVG paint: 0
- Typography canonical selectors: 1070
- Typography source files scanned: 164
- production generation: 44/44
...
Current data path:
- page server-loads from getLivePublicStackGraph()
- public live API exists under /api/stack/live
- current visible graph is 45 nodes / 71 links / 7 rooms
- hierarchy remains House → Room → Object
- this hierarchy and topology logic are considered GOOD by the owner
...
Current wiring legend semantics:
- PRIVATE FABRIC
- DATA BUS
- HTTP / APP
- CONTROL
...
Do not rewrite the graph model merely for visual reasons.
Do not change room membership, link semantics, live-state logic, or navigation behavior unless a real data/logic bug is found.
The current task area is presentation refinement, not topology redesign.

# 58. SYSTEMS ATLAS — CURRENT VISUAL CHECKPOINT

The CURRENT desired Stack presentation is:
“Systems Atlas PRE-ICON + GLOBAL NAV”.

This is the state immediately BEFORE the later experiment that added full SVG room glyphs and service pictograms.
It preserves the Systems Atlas layout/card/wiring/footer design that the owner liked.

Current visible characteristics:
- Dark mode: tinted room cards with colored borders/rails and luminous semantic wiring.
- Normal mode: white room cards with colored borders/rails and colored semantic wiring.
- Top toolbar: LIVE + STACK HOUSE + small operating subtitle + node/link/room counts.
- No DISCOVERY/FRESH telemetry box.
- Systems Atlas footer remains.
- Wiring Legend and Room Index remain.
- ENTER ROOM plate remains.
- Room badge remains the PRE-ICON marker style, not the later full pictogram set.
- Node chips remain the pre-icon text/initial chip style, not service-type pictograms.
- No RoomGlyph component.
- No NodeChipGlyph component.

The owner explicitly corrected a rollback mistake and requested THIS exact checkpoint.
Do not roll back to pre-Systems-Atlas Stack unless explicitly asked.

# 59. GLOBAL NAV CONTRACT — IMPORTANT

The owner noticed /stack navigation looked different from the rest of the site.
Root cause was Stack-specific CSS overriding the shared SiteHeader.

Those Stack-only nav overrides have now been removed.

Measured dark-mode parity at viewport 1536×960:
- /writing shell: x=48, width=1440
- /projects shell: x=48, width=1440
- /stack shell: x=48, width=1440

Shared header metrics now match:
- header height: ~60.234px
- padding: 8px 12px
- border radius: 14px
- nav gap: 4px
...
- brand strong: 14px
- brand small: 11px
- waveform cluster: display flex
- header-state height: ~18.14px
- header-state padding: 0
- search trigger width: 34px

RULE:
- /stack must use the SAME global SiteHeader as Writing/Projects/Home.
- Do not add .stack-page overrides for header sizing, brand sizing, nav link sizing, waveform visibility, search width, or status pill geometry.
- If global nav changes, fix the shared component/global styles, not Stack-only CSS.

# 60. STACK FILES TO READ FIRST

Primary files:
- src/app/stack/page.tsx
- src/components/stack/stack-spatial.tsx
- src/app/globals.css
- src/lib/stack-graph.ts
- src/server/stack/public.ts
- src/app/api/stack/live/route.ts

Current Stack page contract:
- uses the shared SiteHeader
- no legacy Stack intro block
- renders StackSpatial directly under main
- managed section visibility still applies

Current CSS markers:
- around line ~6238: “SYSTEMS ATLAS PRE-ICON CHECKPOINT / 2026-09-19”
- around line ~6484: “SYSTEMS ATLAS DEMO MAP — PRE ICON PASS”

Those markers identify the current visual layer.
There are older Stack rules earlier in globals.css; the later Systems Atlas rules intentionally override them.
Do not casually delete earlier Stack CSS without checking Room/Object behavior and both themes.

Current stack-spatial component still contains the production House / Room / Object logic.
Only presentation around the graph was changed in the recent redesign cycle.

# 61. RECENT STACK REDESIGN TIMELINE / WHAT NOT TO REPEAT

Sequence:
1. Original Stack was functionally strong, but owner felt Normal mode was visually dirty / not bright / not premium enough.
2. “Calm Atlas” pass reduced contrast too much. Owner rejected it as washed-out.
3. “Systems Atlas” direction was created from visual mockups and was accepted conceptually.
4. A later pass added full room SVG glyphs + service pictograms and diverged Dark vs Normal too much.
5. Search/global nav also got Stack-specific overrides; owner rejected that divergence.
6. A rollback then went TOO FAR back to pre-Systems-Atlas Stack; owner corrected this.
7. Current state was reconstructed to the exact PRE-ICON Systems Atlas checkpoint.
8. Global nav was then restored to the shared site contract.

Lessons:
- Do not solve polish by lowering opacity across the whole system.
- Do not let Dark and Normal become different component structures.
- Do not redesign global navigation inside a route-specific stylesheet.
- Do not add room/service icon systems unless both themes use one coherent contract and owner explicitly asks.
- Preserve the current graph layout and information density; the owner likes its logic.

# 62. PERSISTENT CHECKPOINTS

A persistent checkpoint has been saved inside the repo:
...
This checkpoint represents the CURRENT desired Stack state:
- Systems Atlas
- pre full-icon experiment
- global nav restored

Temporary copies also exist in /tmp:
- /tmp/stack-spatial.systems-atlas-pre-icon-global-nav.tsx
- /tmp/globals.systems-atlas-pre-icon-global-nav.css
- /tmp/stack-page.systems-atlas-pre-icon-global-nav.tsx

Do not rely only on /tmp because it is volatile.
Prefer the persistent docs/checkpoints copy if a later session needs a surgical rollback.

Other older /tmp backups from the redesign cycle may still exist, including pre-Calm and pre-Systems-Atlas variants.
Those are historical only and must NOT be treated as the current target.

# 63. HOME-DEMO / HERO-DEMO-2 CURRENT STATE

Recent public-home exploration lives mainly in:
- src/app/home-demo/page.tsx
- src/app/hero-demo-2/page.tsx
- src/components/demo/rack-server-visual-v2/
- src/components/home/top-atmosphere.tsx

/home-demo currently integrates RackServerVisualV2 directly into the hero.
It also includes:
- live organism runtime summary
- NOW strip
- selected projects
- field notes / Writing
- Music Sensor instrument
- Living Field / LivingCanvas
- closing statement

/hero-demo-2 is the focused rack-hero prototype route.
It uses the same RackServerVisualV2 and current managed home.hero content.

RackServerVisualV2 is CODE, not an image.
It is SVG/React/CSS with:
- isometric rack cabinet
- internet / storage / edge nodes
- physical-looking cable runs
- moving signal pulses
- randomized asymmetric LEDs
- occlusion masks so rear cables can pass behind the cabinet
- perspective floor grid
- cabinet top/side vents and current tuned geometry

# 64. PRODUCTION HOME VS DEMO HOME

Important:
- production root / is NOT switched to /home-demo
- current production root still uses the existing managed Home implementation in src/app/page.tsx
- production Home still uses TopAtmosphere + shared SiteHeader + current LivingCanvas hero/runtime surfaces
- /home-demo is an experimental candidate layout
- /hero-demo-2 is a focused experimental hero route

Do not silently replace production / with /home-demo.
The owner has been iterating visually and wants to approve the demo direction first.

Current shared header component:
- src/components/site/header.tsx
- SiteSearch
- ThemeSwitch
- MusicWaveIndicator
...
Global-nav parity is now a hard requirement.
Route-specific pages should use SiteHeader without route-specific geometry overrides.

Recent home-demo design direction:
- editorial public systems lab
- normal Vietnamese blog language
- project + Writing + live-system context
- code-rendered rack hero, not a static generated image

# 65. PUBLIC SERVING / DEV SERVER — IMPORTANT

At handoff time, thaiduy.digital is currently being served through the local Next dev listener on port 3000.

A 502 was observed when no process was listening on :3000.
The route recovered immediately after restarting:
```bash
cd /home/ubuntu/n8n2erpnext/thaiduy.digital
nohup bun dev > /tmp/thaiduy-digital-bun-dev.log 2>&1 &
```

Current listener:
- Next.js dev / Turbopack
- port 3000
- log: /tmp/thaiduy-digital-bun-dev.log
- next-server PID observed after restart: 2653984

Public smoke after restart:
- / → 200
- /home-demo → 200
- /hero-demo-2 → 200
- /writing → 200
...
If public returns 502, FIRST check `ss -ltnp | grep :3000` and the bun dev log before changing code or proxy config.

# 66. LATEST VERIFIED QA STATE

Latest full QA after restoring global nav parity:
```bash
bunx eslint src/components/stack/stack-spatial.tsx src/app/stack/page.tsx
bun run theme:check
bun run layout:check
bun run typography:check
git diff --check -- src/components/stack/stack-spatial.tsx src/app/stack/page.tsx src/app/globals.css
bun run build
```
...
- git diff --check PASS
- Next production build PASS
- 44/44 static generation steps PASS

The last build was run after:
- Systems Atlas pre-icon reconstruction
- global SiteHeader parity restoration
- persistent checkpoint creation

Do not assume a later edit still passes these gates.
Re-run the same set before declaring a Stack/UI change finished.

# 67. CURRENT WORKTREE / VERSION-CONTROL WARNING

Current Git branch:
- main

Current HEAD:
- a2352940f7e063d3c8b53a07512b4854f1ae69fd
- commit subject: fix: serialize traffic query timestamps

origin/main points to the same commit, but this DOES NOT represent the current working product.
The worktree contains a large amount of intentional uncommitted work.

Major dirty areas include:
- CMS/public content management
- Writing system and engagement
- tag registry and migrations
- theme/typography/layout audit scripts
- current Home work
- Music Sensor
- Stack runtime and Stack UI
- control surfaces
- search/legal/projects detail
- hero/home demo routes
- new docs and migrations

Do not make one giant “cleanup” commit without first reviewing scope.
Do not reset unrelated dirty files while working on Stack or Home.
Use surgical edits and targeted diff review.

Before any branch/merge/reset operation, inspect:
```bash
git status --short
git diff --stat
...
# 68. OWNER VISUAL / UX PREFERENCES LEARNED THIS SESSION

The owner is satisfied with the site concept, logic, features, and prescribed fonts.
The ongoing dissatisfaction is mainly layout/UI/color quality and visual polish.

Strong preferences observed:
- bright/clean/premium Normal mode; not muddy pastel
- Dark can be cinematic, but must remain crisp and readable
- one shared component structure across Dark and Normal
- global navigation must look identical across routes
- avoid route-specific nav experiments
- technical visuals should feel engineered, not decorative
- wiring/endpoints must be physically and visually plausible
- motion should mimic real systems, not synchronized decorative animation
- natural Vietnamese blog language; avoid rigid spec/handoff tone on public Writing

Specific negative reactions:
- “Calm Atlas” was too washed out
- icon divergence between Dark/Normal was rejected
- changing Stack search/nav independently was rejected
- rolling back all the way to old Stack was rejected
- adding visual complexity without matching the approved mockup was rejected

Preferred workflow:
- prototype on demo route
- owner visually approves
- then merge into target route
- after approval, make small surgical refinements rather than another wholesale redesign

# 69. RECOMMENDED NEXT-SESSION START SEQUENCE

1. Read this file from section 55 onward first.
2. Confirm device/repo:
   - /home/ubuntu/n8n2erpnext/thaiduy.digital
   - branch main
3. Run:
```bash
git status --short
ss -ltnp | grep :3000
tail -50 /tmp/thaiduy-digital-bun-dev.log
```
4. Open public:
   - https://thaiduy.digital/
   - https://thaiduy.digital/home-demo
   - https://thaiduy.digital/stack
5. For Stack work, compare current files against:
   - docs/checkpoints/2026-09-20-systems-atlas-pre-icon-global-nav/
6. Do not touch topology/data logic unless the owner reports a logic bug.
7. If changing visual Stack code, verify both Dark and Normal with the same viewport.
8. Confirm global nav parity against /writing or /projects after every Stack CSS change.
9. Run targeted ESLint + theme/layout/typography + build before closeout.
10. Append a new handoff section after meaningful changes.

Current likely continuation:
- continue visual refinement of the public site
- keep Systems Atlas stable
- continue evaluating /home-demo before any production-home role swap

# 70. GOLDEN CHECKPOINT AUDIT / CONSOLIDATION — 2026-09-20

The owner requested one final PR-style code review before consolidating the entire dirty worktree into a GitHub Golden checkpoint.

Review lifecycle:
- temporary branch: `review/golden-audit-20260920`
- snapshot commit: `12a732da` (`chore: pre-golden full product review snapshot`)
- audit-fix commit: `4ac495e0` (`chore: resolve pre-golden audit findings`)
- branch was pushed to GitHub for remote compare
- GitHub connector could not create the PR because the integration returned HTTP 403 `Resource not accessible by integration`
- the VPS had no `gh` CLI installed
- GitHub reported no workflow runs/status checks for the review commit
- therefore acceptance used the pushed branch comparison plus full local lint/type/audit/build gates
- after review, the remote and local review branch were deleted as requested
- local `main` was fast-forwarded to the reviewed state before final cleanup

Important audit findings fixed:
- removed unused `@aws-sdk/s3-request-presigner`
- added explicit direct dependency `server-only`
- removed unused old-Home `ActivityRail` and `EntityConsole`
- removed verified dead entity/activity/old-organ CSS while retaining shared primitives still used
- removed stale generated `runtime/` data and ignored `/runtime/`
- removed unresolved SVG filter reference `url(#rackv2-soft)`
- fixed CSS-module import after moving the production rack visual
- normalized whitespace/CRLF issues found by `git diff --check`

# 71. CANONICAL HOME / DEMO REMOVAL

The former `/home-demo` design is now the only canonical production Home at `/`.

All experimental demo routes were removed completely:
- `/home-demo`
- `/hero-demo`
- `/hero-demo-2`
- `/stack-demo`

There are no source references to those route names after cleanup.
The previous `home-demo-*` CSS/class namespace was renamed to `home-public-*` so production code no longer presents itself as a demo.

Rack visual cleanup:
- old V1 demo rack component was deleted
- production V2 rack was moved out of `src/components/demo/`
- canonical path is now `src/components/home/rack-server-visual/rack-server-visual.tsx`
- canonical export is `RackServerVisual`

The Home atmospheric transition fix remains canonical:
- `TopAtmosphere` extends past the Hero boundary
- Dark/Normal use a longer fade into `var(--bg)`
- Hero has a bottom blend bridge rather than a hard visual cutoff

# 72. GOLDEN ACCEPTANCE / ROLLBACK CONTRACT

Final pre-commit acceptance:
- ESLint: 0 errors; 13 known `@next/next/no-img-element` warnings
- `bunx tsc --noEmit`: PASS after removing stale `.next` route types
- Theme audit: PASS; 460 Normal-mode selectors; all uncovered counters = 0
- Layout audit: PASS
- Typography audit: PASS; 1047 canonical selectors / 154 source files
- `git diff --check origin/main`: PASS
- Next production build: PASS
- static generation: 40/40
- build route manifest contains no demo routes

Golden tag contract:
- `golden-2026-09-20-public-site`
- tag must point to the single consolidated Golden commit on `main`
- use this tag for full-repository rollback
- keep the older Stack-only checkpoint for surgical `/stack` rollback

Do not recreate demo routes unless the owner explicitly requests a new isolated prototype route.
